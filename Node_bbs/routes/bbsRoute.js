...
router.get("/write", function (req, res) {
  ...
  // insert를 수행할때 비어있는 vo를 만들어서 bbsWrtie에 전달해줘야 한다.
  let data = new bbsVO();
...
router.post("/write", function (req, res) {
  ...
  // form에 전송받은 데이터를 통째로 bbs(data) 객체로 생성하라
  // data 에는 form에서 입력하여 전송한 데이터 (작성자, 제목, 내용)과
  // 서버에서 임의로 추가한 데이터(날짜, 시간, 카운트)를 모두 모아서
  // 하나의 VO 객체로 생성을 하고
  // 거기에 DB CRUD와 관련된 함수들을 함께 추가하여 객체 생성함
...
deleteMany(조건문)은 조건에 일치하는 데이터를 모두 삭제
...
하지만 update를 수생할때 id값을 URL에 전닥하여 보냈기 때문에 
for(method="POST",actipn="/bbs/update/id값") 형태의 URL이 자동으로 만들어져
  ...
  bbsVO
    .updateOne(
      { _id: id }, //where 조건문
      {
        ...
        // 주의 해야하는 코드
        // $set {req.body}
        // update를 수행해야하는데 실제로 form에서 전달된 데이터외에
